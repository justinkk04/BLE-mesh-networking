...
namespace {
    ...
    constexpr gpio_num_t I2C_SDA_PIN = GPIO_NUM_6; // Wire to INA260 SDA
    constexpr gpio_num_t I2C_SCL_PIN = GPIO_NUM_7; // Wire to INA260 SCL
    constexpr uint32_t   I2C_FREQ_HZ = 400000;

    // INA260 valid address range: 0x40-0x4F (depends on A0/A1 wiring)
    constexpr uint8_t INA260_ADDR_MIN = 0x40;
    constexpr uint8_t INA260_ADDR_MAX = 0x4F;

    uint8_t ina260_addr = 0; // Auto-detected at runtime

    // INA260 registers
    constexpr uint8_t  INA260_REG_CONFIG  = 0x00;
    constexpr uint8_t  INA260_REG_CURRENT = 0x01;
    constexpr uint8_t  INA260_REG_VOLTAGE = 0x02;
    constexpr uint16_t INA260_CONFIG      = 0x6727; // 1024-sample averaging

    bool ina260_ok = false; // Set true if INA260 found on I2C
}
...
void i2c_scan(void) {
  ...
  if (found == 0) {
    ESP_LOGW(TAG,
             ...
             static_cast<int>(I2C_SDA_PIN), static_cast<int>(I2C_SCL_PIN));
  }
}

esp_err_t sensor_init(void) {
  // Initialize I2C master — zero-init then assign nested .master.clk_speed
  i2c_config_t conf = {};
  conf.mode          = I2C_MODE_MASTER;
  conf.sda_io_num    = static_cast<int>(I2C_SDA_PIN);
  conf.scl_io_num    = static_cast<int>(I2C_SCL_PIN);
  ...
  ESP_LOGI(TAG, "I2C initialized: SDA=GPIO%d, SCL=GPIO%d, %lu Hz",
           static_cast<int>(I2C_SDA_PIN), static_cast<int>(I2C_SCL_PIN),
  ...
  if (ina260_addr != 0) {
    ...
    uint8_t config_data[3] = {INA260_REG_CONFIG,
                              static_cast<uint8_t>((INA260_CONFIG >> 8) & 0xFF),
                              static_cast<uint8_t>(INA260_CONFIG & 0xFF)};
    i2c_cmd_handle_t cmd = i2c_cmd_link_create();
    i2c_master_start(cmd);
    i2c_master_write_byte(cmd, static_cast<uint8_t>((ina260_addr << 1) | I2C_MASTER_WRITE), true);
...
float ina260_read_voltage(void) {
  ...
  i2c_master_write_byte(cmd, static_cast<uint8_t>((ina260_addr << 1) | I2C_MASTER_READ), true);
