...
**Version:** v0.7.1 (Web Dashboard & Universal Nodes)
**Status:** Stable / In Development

A self-healing BLE Mesh network for remote DC power monitoring and control. The system uses ESP32-C6 "universal nodes" that each function as both a sensor AND a GATT gateway, bridged to a Raspberry Pi 5 for automated power management, automatic failover, and real-time web dashboard monitoring.
...
- **Web Dashboard:** Native FastAPI web server integrated alongside the terminal UI, providing real-time SVG topology, metrics, and multi-chart telemetry via WebSocket.
...
- `gateway-pi5/gateway-code/` — Python 3 gateway (`gateway.py` entry point + FastApi `web_server.py`, `dc_gateway`, `tui_app`, `power_manager`, `ble_thread`, SQLite `db.py`, `node_state`, and constants).
- `gateway-pi5/dashboard/` — Frontend web UI (`index.html`, `css/style.css`, and JS modules).
...
- **v0.7.0 (Complete):** Gateway failover — universal nodes, auto-reconnect, PM mesh fixes. Dedicated GATT gateway deprecated.
- **v0.7.1 (Current):** Responsive Web Dashboard — native FastAPI backend, WebSocket streaming, D3.js topology, and time-series SQL logging seamlessly integrated with TUI.
